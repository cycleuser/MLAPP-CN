...
定义一个函数$F(q) \overset\triangle{=} p(X\le q)$，这就是 X 的连续分布函数（cumulative distribution function，缩写为 cdf）。很明显这个 cdf 是一个单调递增函数（monotonically increasing function）。如图2.3（a）所示。来利用这个记号则有：
